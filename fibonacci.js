...
const btnTecnica1 = document.querySelector(".check-fibonacci");
const resTecnica1 = document.querySelector("#result1");

btnTecnica1.addEventListener("click", function () {
  const num = parseInt(document.getElementById("number").value);
  let isFibonacci = fibonacci(num);

  if (isFibonacci) {
    resTecnica1.textContent = `O valor ${num} faz parte da sequencia de Fibonacci`;
  } else {
    resTecnica1.textContent = `O valor ${num} não faz parte da sequencia de Fibonacci`;
  }
});
